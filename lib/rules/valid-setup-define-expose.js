    ...
    return utils.defineTemplateBodyVisitor(
      context,
      {
        ...
        VAttribute(node) {
          ...
          // wx:for-item / wx:for-index 循环变量自动暴露
          // @ts-ignore
          if (
            node.key &&
            node.key.name &&
            typeof node.key.name === 'object' &&
            ['for-item', 'for-index'].includes(node.key.name.name) &&
            node.value &&
            node.value.expression &&
            node.value.expression.value
          ) {
            // @ts-ignore
            exposeSet.add(node.value.expression.value)
          }
      ...
      },
      {
        ...
        CallExpression(node) {
          ...
          // @ts-ignore
          if (node.callee.name === 'onReactHooksExec') {
            const arg = node.arguments[0]
            if (
              arg &&
