  ...
  excuteOnMpxCreateApp(context, cb) {
    return {
      ...
      'ObjectExpression:exit'(node) {
        const type = getMpxObjectType(context, node)
 ...
 */
function getMpxObjectType(context, node) {
  if (node.type !== 'ObjectExpression') {
    return null
  }
  const parent = getParent(node)
  if (parent.type === 'CallExpression' && skipTSAsExpression(parent.arguments.slice(-1)[0]) === node) {
